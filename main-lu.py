...
#     await message.answer(f'Пока, {message.from_user.full_name}!')  # Отправляем приветственное сообщение
#
# @dp.message(Command(commands=['info']))  # Декоратор для обработки команды /start
# async def start(message: types.Message):
#     await message.answer(f'Я бот, есть всего 3 команды: start-stop-info')  # Отправляем приветственное сообщение
#
...
async def send_welcome(message: types.Message):
    await message.answer(f'Привет, Луиза! Чем я могу тебя порадовать сегодня? Напиши /compliment или /quote 🌹')
...
async def send_goodbye(message: types.Message):
    await message.answer(f'Пока, Луиза! Всегда рад тебя видеть 💖')
